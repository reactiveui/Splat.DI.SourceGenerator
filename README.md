...
- Compiler: .NET SDK 9.0.300 or later, or Visual Studio 2022 17.14 or later (the generator is built against Roslyn 4.14)
- Language: C# 8.0 or later (the generated code uses `#nullable` directives)
...
|--------------|----------|-------------|----------|
| SPLATDI001 | Error | Multiple constructors without `[DependencyInjectionConstructor]` attribute | Adds attribute to selected constructor |
...
| SPLATDI005 | Error | Registered types depend on each other through their constructors in a cycle | Manual fix required |
| SPLATDI006 | Warning | A service is registered more than once with the same contract | Manual fix required |
| SPLATDI007 | Error | A constructor takes `Lazy<T>` of a service that is not registered with `RegisterLazySingleton` | Manual fix required |

The analyzer detects issues in real-time and offers automatic fixes via Quick Actions (Ctrl+. or Cmd+.). SPLATDI005 to SPLATDI007 need every registration in the project, so the source generator reports them when it runs.
...
The code generated for the [Complete Example](#complete-example):

```csharp
static partial void SetupIOCInternal(Splat.IDependencyResolver resolver)
{
    resolver.Register<global::ILogger>(() => new global::FileLogger());

    resolver.Register<global::IUserService>(
        () => new global::UserService(
            resolver.GetService<global::IDatabase>() ?? ThrowNotRegistered<global::IDatabase>("global::IDatabase"))
        {
            Logger = resolver.GetService<global::ILogger>() ?? ThrowNotRegistered<global::ILogger>("global::ILogger"),
        });

    var lazy0 = new global::System.Lazy<global::IDatabase>(() => new global::SqliteDatabase());
    resolver.Register<global::System.Lazy<global::IDatabase>>(() => lazy0);
    resolver.Register<global::IDatabase>(() => lazy0.Value);
}
```

Transient registrations are made before lazy singletons. A missing dependency throws an `InvalidOperationException` naming its type, from a `ThrowNotRegistered` helper written into the same file.

## Performance

- No reflection at run time: resolving a service allocates only the objects it constructs, and a registration with no dependencies creates its factory delegate once
- Full Native AOT and trimming support
- Builds: a generation pass over 128 registrations takes 9.6 ms and allocates 1.8 MB, and an IDE edit elsewhere in the project reruns it in 8.2 ms. An edit that only moves registrations does not regenerate the file

The benchmarks, and how to reproduce these figures, are in [src/benchmarks/README.md](src/benchmarks/README.md).
...
See the [ReactiveUI contribution guide](https://www.reactiveui.net/contribute/index.html) for details.
