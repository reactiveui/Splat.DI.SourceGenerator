...
public class GeneratorCodeGenerationTests
{
    ...
    /// <summary>
    /// Tests that the extension methods file (Constants.ExtensionMethodText) includes GeneratedCodeAttribute with version.
    /// This is the ONLY place where GeneratedCodeAttribute should appear on the SplatRegistrations partial class.
    /// </summary>
    /// <returns>A task representing the asynchronous test operation.</returns>
    [Test]
    public async Task ExtensionMethodText_IncludesGeneratedCodeAttribute()
    {
        var result = Constants.ExtensionMethodText;
    ...
    /// <summary>
    /// Tests that GenerateSetupIOCMethod does NOT include GeneratedCodeAttribute to avoid duplication.
    /// The GeneratedCodeAttribute should only appear once on the partial class (in Splat.DI.g.cs),
    /// not on both partial class declarations, which would cause CS0579 duplicate attribute error.
    /// </summary>
    /// <returns>A task representing the asynchronous test operation.</returns>
    [Test]
    public async Task GenerateSetupIOCMethod_DoesNotIncludeDuplicateGeneratedCodeAttribute()
    {
        var transients = ImmutableArray<TransientRegistrationInfo>.Empty;
        var lazySingletons = ImmutableArray<LazySingletonRegistrationInfo>.Empty;
        ...
        // The GeneratedCodeAttribute should NOT be in the Splat.DI.Reg.g.cs output
        // It should only be in Splat.DI.g.cs (from Constants.ExtensionMethodText)
        await Assert.That(result).DoesNotContain("[global::System.CodeDom.Compiler.GeneratedCodeAttribute(");
    }
}
